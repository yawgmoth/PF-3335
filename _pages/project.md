...
In the project for the class you will develop a simple planner in python. To do this, you will develop four separate modules, each with its own requirements and deadline:
 ...
 1. A representation of logical formulas, due ??, worth 15 points
 ...
 5. A separate submission to our own internal planning competition (optional)
...
The project has to be developed in [python 3](http://python.org), using the framework found [here](http://github.com). The framework requires only a standard python 3 installation, and you are also only allowed to use the python standard libraries. 
...
The domain file defines types, constants, predicates, and operators (action schemata). Its root node is called `define`, under which there are several sections, some of which are optional:
  ...
  - The `:types` section defines which types (sets of objects) exist in the domain (optional).
...
Your parser should be implemented in the file `pddl.py`, and has to implement two functions:
   ...
   - `parse_domain` is passed the file name of a PDDL domain file, and returns a parsed representation of that domain. I recommend returning a list of an action schemata representation, a dictionary mapping types to sets of 
     constants for each type. Take care to include an extra mapping from the type `""` to a set of all objects.
...
The second part of the assignment requires you to implement the actual planning process in `planner.py`. This process entails two parts:
...
of these problems increases with increasing numbers, but your planner should be able to solve the first 10 or 11 in less than 5 seconds. In the next task we will look at how to improve the performance of the 
planner.
